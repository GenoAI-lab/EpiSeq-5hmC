"""
EpiSeq-5hmC: Leave-One-Tissue-Out (LOTO) training.

Trains on 6 tissues, validates on held-out chromosomes within those 6 tissues.
The excluded tissue is evaluated separately via calibrate.py.

Usage:
    python train_loto.py --exclude_tissue Liver --split_idx 0
    python train_loto.py --exclude_tissue Cerebellum --split_idx 0 --config config.yaml
"""
...
DEFAULT_CONFIG = {
    ...
    'hardware': {'gpu_id': 0, 'num_workers': 8},
}
...
# ==============================================================================
def get_args():
    p = argparse.ArgumentParser(description="EpiSeq-5hmC LOTO Training")
    p.add_argument("--exclude_tissue", type=str, required=True,
                   help="Tissue to exclude (e.g., 'Liver', 'Cerebellum')")
    p.add_argument("--split_idx", type=int, default=0,
                   help="Chromosome split index for train/val within remaining tissues")
...
def main():
    ...
    save_path = args.save_name or os.path.join(
        P['output_dir'], f"model_loto_no_{args.exclude_tissue}_cv{args.split_idx}.pth"
    )

    print("=" * 70)
    print(f"LOTO Training | Exclude: {args.exclude_tissue} | "
          f"Mode: {args.mode} | Split: {args.split_idx}")
    print("=" * 70)

    # Data — exclude one tissue
    val_chroms = load_split_chroms(P['split_file'], args.split_idx)
    sample_map, sample_ids = prepare_sample_map(
        P['sample_tsv'], features=M['features'], exclude_tissue=args.exclude_tissue,
    )
    ...
    print(f"  Samples: {len(sample_ids)} | Train: {len(train_idx):,} | Val: {len(val_idx):,}")
    ...
    for epoch in range(1, T['max_epochs'] + 1):
        ...
        print(f"[LOTO] Epoch [{epoch}/{T['max_epochs']}] "
    ...
    print(f">>> Finished LOTO (excluded: {args.exclude_tissue})")
